...
# 16. VERSION CONTROL — REPO SETUP (FIRST TASK)

## Current State: No Version Control

There is NO git repo, NO version history, NO branches. All code lives as loose files in `/mnt/user-data/outputs/`. Each session overwrote the previous file. We have no way to diff v2.5.0 vs v2.6.4 — they're gone.

**First order of business: Create a GitHub repo and commit everything.**
...
├── LICENSE                             # TBD — probably proprietary
├── .gitignore
├── requirements.txt                    # PyQt6, etc.
├── setup.py                            # (or pyproject.toml)
│
├── fop/                                # Main application package
│   ├── __init__.py
│   ├── main.py                         # Entry point (thin wrapper)
│   ├── app.py                          # FileOrganizerPro QMainWindow class
│   ├── colors.py                       # Colors class
│   ├── constants.py                    # VERSION, PROMPT_PRESETS
│   ├── models.py                       # FileInfo, OrganizationPlan, enums
│   ├── scanner.py                      # FileScanner QThread
│   ├── classifier.py                   # FileClassifier QThread
│   ├── metadata.py                     # MetadataReader (exiftool wrapper)
│   ├── llm_client.py                   # LLM API calls (NEW)
│   ├── executor.py                     # File copy/move execution (NEW)
│   ├── catalog_search.py              # Multi-catalog search engine
│   ├── catalog_search_dialog.py       # Catalog search UI
│   └── dialogs/
│       ├── __init__.py
│       ├── preferences.py             # PreferencesDialog
│       ├── execute.py                 # ExecuteDialog
│       └── export.py                  # ExportDialog
│
├── tests/
│   ├── test_catalog_search.py
│   ├── test_classifier.py             # TODO
│   ├── test_scanner.py                # TODO
│   └── test_models.py                 # TODO
│
├── docs/
│   ├── developer_handoff.md           # This document
│   ├── lrforge_plugin_requirements.md
│   ├── status_reports/
│   │   └── 2026-02-03.md
│   └── session_logs/
│       ├── 2026-01-30.md
...
│
├── mockups/
│   ├── fop_mockup.html                # Original neumorphic reference
│   └── fop_complete_mockup.html       # Full 3-page mockup
│
├── lrforge/                            # Future — Lightroom plugin
│   ├── LrForge.lrplugin/
│   │   ├── Info.lua
│   │   ├── dkjson.lua
│   │   └── ... (not yet written)
...
│
└── scripts/
    ├── build.sh                        # PyInstaller packaging (future)
    └── catalog_search_demo.py         # Standalone demo
```

## Step-by-Step: Create Repo

```bash
# 1. Create repo on GitHub
#    Name: fileorganizerpro
#    Private repo (it's a commercial product)
#    No template, no README (we'll push our own)

# 2. Initialize locally
mkdir fileorganizerpro && cd fileorganizerpro
git init
git branch -M main
...
# 5. Copy existing files into structure
#    Option A: Single-file for now (fastest, ship today)
cp FileOrganizerPro.py fop/
cp catalog_search.py fop/
cp catalog_search_dialog.py fop/
cp test_catalog_search.py tests/

#    Option B: Split into modules (cleaner, takes 4-6 hours)
#    See "Module Split Plan" below

# 6. Initial commit with ALL existing work
git add -A
git commit -m "feat: initial commit — FOP v2.6.4 + catalog search

FileOrganizerPro v2.6.4 (2,390 lines):
- Complete 3-page UI (Welcome, Setup, Results)
- File scanning with recursive directory walk
- Rule-based classification (50+ patterns)
- Keyword classification from IPTC/XMP metadata
- Duplicate detection (hash-based)
- Export to CSV, TXT, Shell Script, .fopplan
- LLM connection test (socket + HTTP)
- Preferences dialog

Multi-Catalog Search (ready to integrate):
- catalog_search.py — SQLite search engine
- catalog_search_dialog.py — PyQt6 UI dialog
- Unit tests passing

NOT yet implemented:
- LLM classification API calls
- File copy/move execution
- Settings persistence (QSettings)
- LrForge plugin (spec only)"

# 7. Push to GitHub
git remote add origin git@github.com:YOURUSERNAME/fileorganizerpro.git
git push -u origin main
```

## Branching Strategy

Keep it simple — solo developer, no need for gitflow:

```
main              ← stable, shippable code
  └── dev         ← active development (optional, can just use main)
      ├── feature/llm-classification
      ├── feature/file-execution
      ├── feature/settings-persistence
      └── feature/lrforge-plugin
```

Suggested workflow:
1. Work on `main` for small fixes and integration
2. Use feature branches for multi-hour work (LLM, execution, LrForge)
3. Tag releases: `v2.6.5`, `v2.7.0`, `v1.0.0`

```bash
# Tag current state
git tag -a v2.6.4 -m "Pre-repo state: UI complete, rules working, no AI"

# After integrating catalog search
git tag -a v2.6.5 -m "Add multi-catalog search feature"

# After LLM + execution
git tag -a v2.7.0 -m "AI classification and file execution working"
```

## Module Split Plan (When Ready)

The single-file `FileOrganizerPro.py` should eventually be split. Here's the mapping:

| Current Lines | New File | Contents |
|---------------|----------|----------|
| 1-60 | `constants.py` | Imports, VERSION |
| 61-82 | `colors.py` | Colors class |
| 83-170 | `constants.py` | PROMPT_PRESETS |
| 171-220 | `models.py` | TrustLevel, ClassificationSource, Confidence, FileInfo, OrganizationPlan |
| 221-320 | `models.py` | (continued) |
| 321-450 | `metadata.py` | MetadataReader |
| 451-550 | `scanner.py` | FileScanner |
| 551-630 | `classifier.py` | FileClassifier |
| 631-880 | `dialogs/preferences.py` | PreferencesDialog |
| 881-1000 | `dialogs/execute.py` | ExecuteDialog |
| 1001-1100 | `dialogs/export.py` | ExportDialog |
| 1101-2390 | `app.py` | FileOrganizerPro (main window) |
| (new) | `main.py` | Entry point: `from fop.app import ...` |
| (new) | `llm_client.py` | LLM API calls |
| (new) | `executor.py` | File copy/move logic |

**Recommendation:** Do the split AFTER LLM + execution are working. Don't refactor and add features at the same time.
...
```
feat: add LLM classification via LM Studio API
fix: preferences dialog layout — URL input on own row
refactor: split FileOrganizerPro.py into modules
docs: update handoff document with git setup
test: add classifier unit tests
